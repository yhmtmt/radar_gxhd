...
void GarminxHDReceive::ProcessFrame(const uint8_t *data, size_t len)
{
  ...
  radar_state->set_state(RadarState_TRANSMIT);
  ...
  pfilter->m_statistics.tend = time_rec;

  if (m_first_receive)
  {
    m_first_receive = false;
    pfilter->m_statistics.tstart = pfilter->m_statistics.tend;
    long long startup_elapsed =
        (pfilter->m_statistics.tstart - pfilter->GetBootTime()) / MSEC;
    spdlog::info("[{}] First radar spoke received after %lld ms",
  ...
  pfilter->m_statistics.accumulate(packet->line_data);
}
...
SOCKET GarminxHDReceive::GetNewReportSocket()
{
  ...
  if (socket != INVALID_SOCKET)
  {
    ...
    spdlog::info("[{}] Scanning interface {} for data from {}",
                 pfilter->get_name(), addr.c_str(), rep_addr.c_str());
  }
  else
  {
    spdlog::error("[{}] Unable to listen to socket {}",
                  pfilter->get_name(), error.c_str());
  }
  return socket;
}
...
SOCKET GarminxHDReceive::GetNewDataSocket()
{
  ...
  if (socket != INVALID_SOCKET)
  {
    ...
    spdlog::info("[{}] Listening for data on {} from {}",
                 pfilter->get_name(), addr.c_str(), rep_addr.c_str());
  }
  else
  {
    spdlog::info("[{}] Unable to listen to socket {}",
                 pfilter->get_name(), error.c_str());
  }
  return socket;
}
...
bool GarminxHDReceive::Loop()
{
  ...
  */
  ...
  if (r > 0)
  {
    ...
    if (dataSocket != INVALID_SOCKET && FD_ISSET(dataSocket, &fdin))
    {
      ...
      else
      {
        closesocket(dataSocket);
        dataSocket = INVALID_SOCKET;
        spdlog::error("[{}] Illegal frame", pfilter->get_name());
      }
    }
    ...
    if (reportSocket != INVALID_SOCKET && FD_ISSET(reportSocket, &fdin))
    {
      ...
      if (r > 0)
      {
        ...
        if (ProcessReport(data, (size_t)r))
        {
          if (!radar_addr)
          {
            ...
            if (radar_state->get_state() == RadarState_OFF)
            {
              spdlog::info("[{}] Detected at {}", pfilter->get_name(), m_addr.c_str());
              radar_state->set_state(RadarState_STANDBY);
            }
          }
      ...
      {
        spdlog::error("[{}] Illegal report", pfilter->get_name());
  ...
  { // no data received -> select timeout
    if (no_data_timeout >= SECONDS_SELECT(2))
    {
      ...
      if (reportSocket != INVALID_SOCKET)
      {
        closesocket(reportSocket);
        reportSocket = INVALID_SOCKET;
        radar_state->set_state(RadarState_OFF);
        memset(&m_interface_addr, 0, sizeof(m_interface_addr));
        radar_addr = 0;
      }
    }
...
bool GarminxHDReceive::UpdateScannerStatus(int status)
{
  ...
  if (status != m_radar_status)
  {
    ...
    {
    case 2:
      radar_state->set_state(RadarState_WARMING_UP);
      spdlog::info(("[{}] Reports status WARMUP"), pfilter->get_name());
      stat = ("Warmup");
      break;
    case 3:
      radar_state->set_state(RadarState_STANDBY);
      spdlog::info(("[{}] Reports status STANDBY"), pfilter->get_name());
      stat = ("Standby");
      break;
    case 4:
      radar_state->set_state(RadarState_SPINNING_UP);
      pfilter->m_data_timeout = now + DATA_TIMEOUT;
      spdlog::info(("[{}] Reports status SPINNING UP"), pfilter->get_name());
      stat = ("Spinning up");
      break;
    case 5:
      radar_state->set_state(RadarState_TRANSMIT);
      spdlog::info(("[{}] Reports status TRANSMIT"), pfilter->get_name());
      stat = ("Transmit");
      break;
    case 6:
      radar_state->set_state(RadarState_STOPPING);
      pfilter->m_data_timeout = now + DATA_TIMEOUT;
      spdlog::info(("[{}] Reports status STOPPING"), pfilter->get_name());
      stat = ("Stopping");
      break;
    case 7:
      radar_state->set_state(RadarState_SPINNING_DOWN);
      spdlog::info(("[{}] Reports status SPINNING DOWN"), pfilter->get_name());
      stat = ("Spinning down");
      break;
    case 10:
      radar_state->set_state(RadarState_STARTING);
      spdlog::info(("[{}] Reports status STARTING"), pfilter->get_name());
      stat = ("Starting");
      break;
    default:
      spdlog::info(("[{}] Reports status %d"), pfilter->get_name(), m_radar_status);
      {
        stat = ("Unknown status");
      }
      ret = false;
    ...
    spdlog::info("[{}] IP {} {}", pfilter->get_name(), m_addr.c_str(), stat.c_str());
  }
  return ret;
}
...
bool GarminxHDReceive::ProcessReport(const uint8_t *report, size_t len)
{
  ...
  if (len >= sizeof(rad_ctl_pkt_9))
  { //  sizeof(rad_respond_pkt_9)) {
    ...
    { // Auto Gain Mode
      ...
      RadarControlState state = RadarControlState_MANUAL;
      if (m_auto_gain)
      {
        switch (packet9->parm1)
        {
        case 0:
          state = RadarControlState_AUTO_1;
          break;

        case 1:
          state = RadarControlState_AUTO_2; // AUTO HIGH
          break;
    ...
    case 0x0933: // Rain clutter mode
      ...
      case 0:
      {
        m_rain_mode = RadarControlState_OFF;
        return true;
      }
      case 1:
      {
        m_rain_mode = RadarControlState_MANUAL;
        return true;
      }
      }
      break;
    ...
    case 0x0939:
    {
      ...
      case 0:
      {
        m_sea_mode = RadarControlState_OFF;
        return true;
      }
      case 1:
      {
        // Manual sea clutter, value set via report 0x093a
        m_sea_mode = RadarControlState_MANUAL;
        return true;
      }
      case 2:
      {
        ...
        // via 0x093b
        if (m_sea_mode < RadarControlState_AUTO_1)
        {
          m_sea_mode = RadarControlState_AUTO_1;
        }
        return true;
      }
      }
    ...
    case 0x093b:
    {
      ...
      if (m_sea_mode >= RadarControlState_AUTO_1)
      {
        m_sea_mode = (RadarControlState)(RadarControlState_AUTO_1 + packet9->parm1);
        radar_state->set_sea(m_sea_clutter, m_sea_mode);
      }
      return true;
    }
    ...
    case 0x093f:
    {
      ...
      {
        radar_state->set_no_transmit_start(0, RadarControlState_OFF);
        radar_state->set_no_transmit_end(0, RadarControlState_OFF);
      }
      return true;
    }
    case 0x0940:
    {
      ...
      if (m_no_transmit_zone_mode)
      {
        radar_state->set_no_transmit_start(packet12->parm1 / 32, RadarControlState_MANUAL);
      }
      return true;
    }
    case 0x0941:
    {
      ...
      if (m_no_transmit_zone_mode)
      {
        radar_state->set_no_transmit_end(packet12->parm1 / 32, RadarControlState_MANUAL);
      }
      return true;
    }
    case 0x02bb:
    ...
    case 0x0942:
    {
      ...
      if (packet9->parm1 == 0)
      {
        m_timed_idle_mode = RadarControlState_OFF;
      }
      else
      {
        m_timed_idle_mode = RadarControlState_MANUAL;
      }
      return true;
    }
    ...
    case 0x0993:
    {
      ...
      spdlog::info("[{}] Garmin xHD 0x0993: state-change in {} ms",
                   pfilter->get_name(), packet12->parm1);
    ...
    case 0x099b:
    {
      ...
      spdlog::error("[{}] Garmin xHD 0x099b: error:{}", pfilter->get_name(), packet->info);
      return true;
    }
    }
  }
